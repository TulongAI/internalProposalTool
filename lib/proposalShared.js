...
export function renderProposalArticleHTML(p) {
  ...
  var scopeHTML = scopeServices.map(function (k, i) {
    var s = SERVICES[k];
    return '<div class="service-line"><div class="sl-num">' + String.fromCharCode(65 + i) + '</div><div><div class="sl-name">' + esc(s.label) + '</div><div class="sl-blurb">' + esc(s.blurb) + '</div>' +
      (s.meta ? '<div class="sl-meta">' + esc(s.meta) + '</div>' : '') + '</div></div>';
  }).join("");
  if (!scopeHTML) {
  ...
  return '' +
    '<div class="doc-wrap">' +
    '<div class="doc-corner-pill">tulong <span class="pill-x">×</span> ' + esc(p.client.company || "Client") + '</div>' +
    '<article class="doc">' +
    ...
    '</div>' +
    '</article>' +
    '<nav class="doc-scrollspy" id="doc-scrollspy" aria-hidden="true"></nav>' +
    '</div>';
}

// Populates the scroll-position dot nav next to a rendered proposal
// document and wires it up (click-to-jump, highlight-on-scroll). Pure
// browser DOM/IntersectionObserver code — call this client-side only,
// after the HTML from renderProposalArticleHTML() is already in the DOM
// (e.g. after an innerHTML assignment or a React effect).
export function mountDocScrollSpy(root) {
  if (typeof document === "undefined") return;
  var scope = root || document;
  var nav = scope.querySelector("#doc-scrollspy");
  var sections = Array.prototype.slice.call(scope.querySelectorAll("article.doc .doc-section"));
  if (!nav || !sections.length) { if (nav) nav.innerHTML = ""; return; }

  sections.forEach(function (sec, i) {
    if (!sec.id) sec.id = "doc-sec-" + i;
  });

  nav.innerHTML = sections.map(function (sec) {
    return '<button type="button" class="sd" data-target="' + sec.id + '" aria-label="Jump to section"></button>';
  }).join("");

  var dots = Array.prototype.slice.call(nav.querySelectorAll(".sd"));
  dots.forEach(function (dot) {
    dot.addEventListener("click", function () {
      var target = scope.querySelector("#" + dot.getAttribute("data-target"));
